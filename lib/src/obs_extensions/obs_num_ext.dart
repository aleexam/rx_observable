...
extension ObservableIntExt on Observable<int> {
...
extension ObservablenIntExt on Observable<int?> {
  /// Bit-wise and operator.
  ///
  /// Treating both `this` and [other] as sufficiently large two's component
  /// integers, the result is a number with only the bits set that are set in
...
extension ObservableNumExt<T extends num> on Observable<T> {
...
extension ObservableDoubleExt on Observable<double> {
  /// Addition operator.
  Observable<double> operator +(num other) {
    value = value + other;
    return this;
  }
...
extension ObservablenDoubleExt on Observable<double?> {
  /// Addition operator.
  Observable<double?>? operator +(num other) {
    if (value != null) {
      value = value! + other;
