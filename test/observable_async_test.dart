...
void main() {
  group('ObservableAsync<T>', () {
    ...
    test('handles complex objects with custom equality', () async {
      ...
      final obs = ObservableAsync<_ComplexObject>(objA, notifyOnlyIfChanged: true);
      var notificationCount = 0;
    ...
    test('onCancel callback is invoked when last listener is removed', () async {
    ...
    test('v getter/setter aliases work correctly', () {
      final obs = ObservableAsync<int>(42);
      expect(obs.v, 42);
    ...
    test('stream property provides direct access to the underlying stream', () async {
      final obs = ObservableAsync<int>(42);
      var streamValue = 0;
    ...
    test('cancelOnError parameter in addStream', () async {
      ...
      obs.addStream(controller.stream, cancelOnError: true)
        .then((_) {
          completed = true;
        })
        .catchError((_) {
    ...
    test('can be used as a drop-in replacement for StreamController', () async {
      ...
      Future<List<int>> collectStreamValues(StreamController<int> controller) async {
        final values = <int>[];
    ...
    test('handles error events the same as StreamController', () async {
      ...
      obs.stream.listen(
        (_) {},
        onError: (e) => obsError = e
      );
      
      controller.stream.listen(
        (_) {},
        onError: (e) => controllerError = e
      );
    ...
    test('concurrently adding values from multiple isolates handles race conditions', () async {
      final obs = ObservableAsync<int>(0);
      final receivedValues = <int>[];
